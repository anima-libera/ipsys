...
/* Allocate a rg_t object and allocate its lcg_array field according to the
...
/* Initialise the given rg with the given seed. */
static void rg_seed(rg_t* rg, rg_seed_t seed);

/* Return the next raw number of the given lcg. The lcg parameters used here
 * are the ones used by GCC's libc implementation (even though rand is more
 * complex than that). */
static uint32_t lcg_next(uint32_t lcg);

/* Perform a half step on the rg and return a raw uint32_t.
 * Note that the lower bits are not random enough to be considered clean, thus
 * a complete step uses high bits (bit 1 to bit 17 (bit 0 is always 0)). */
static uint32_t rg_raw_uint32(rg_t* rg);

/* Perform a step on the rg and return a clean and well randomized uint32_t. */
...
int rg_int_full(rg_t* rg)
{
	...
	/* *((int*)&x) is ok too but may be slower ? */
}
